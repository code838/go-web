...
import type { ThirdLoginInfo } from '@/types'
...
export default function LoginDialog() {
	...
	const showTelegramWidget = (tgConfig: any) => {
		...
		// 添加提示信息：如果看到 "Bot domain invalid" 错误
		setTimeout(() => {
			...
			if (iframe) {
				...
				const checkError = setInterval(() => {
					...
					// 如果 iframe 还存在但很久没有反应，可能是域名配置问题
				}, 1000)
				
				// 30秒后清除检查
				setTimeout(() => clearInterval(checkError), 30000)
			}
		}, 2000)
	...
	return (
		<DialogShell title={t('title')} close={closeDialog}>
			...
			<div className='mt-6 flex gap-2 rounded-lg border border-white/10 p-1'>
				<button
					...
					className={`flex-1 rounded-md px-4 py-2 text-sm font-medium transition-colors ${
						loginType === 'phone' ? 'bg-brand text-white' : 'text-secondary hover:text-white'
					}`}>
					{t('phoneTab')}
				</button>
				<button
					...
					className={`flex-1 rounded-md px-4 py-2 text-sm font-medium transition-colors ${
						loginType === 'email' ? 'bg-brand text-white' : 'text-secondary hover:text-white'
					}`}>
					{t('emailTab')}
				</button>
			</div>
			...
			<div className='flex items-center justify-between'>
				<div className='text-muted-foreground inline-flex cursor-pointer items-center gap-2 text-sm select-none' onClick={() => setRemember(!remember)}>
					<Checkbox checked={remember} />
					<span>{t('rememberAccount')}</span>
				</div>
				<button type='button' className='text-brand text-sm hover:underline focus:outline-none' onClick={() => openDialog('forgot')}>
					{t('forgotPassword')}
				</button>
			</div>
			</div>

			<button disabled={loading} onClick={handleLogin} className='bg-brand w-full rounded-lg px-4 py-3 font-semibold disabled:opacity-50'>
				{loading ? t('submitting') : t('submit')}
			</button>

		{thirdLoginConfig && thirdLoginConfig.length > 0 && (
			<>
				<div className='gap-2ƒ flex items-center'>
					<div className='flex-1 border-t'></div>
					<span className='text-secondary text-xs'>{t('otherMethods')}</span>
					<div className='flex-1 border-t'></div>
				</div>
			...
			<div className='flex justify-center'>
				<span className='text-secondary text-sm'>
					{t('noAccount')}
					<button type='button' className='text-brand ml-1 hover:underline focus:outline-none' onClick={() => openDialog('register')}>
						{t('registerLink')}
					</button>
				</span>
			</div>
