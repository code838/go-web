...
export default function WithdrawRecordsPage() {
  ...
  const handleCopy = (text: string, e: React.MouseEvent) => {
    ...
    toast.success(t('withdrawRecords.copySuccess'))
  }
  ...
  const getStatusBadge = (status: number) => {
    const statusConfig = {
      1: {
        text: t('withdrawRecords.statusPending'),
        className: 'bg-yellow-500/20 text-yellow-500'
      },
      2: {
        text: t('withdrawRecords.statusCompleted'),
        className: 'bg-[#6741FF33] text-[#6741FF]'
      },
      3: {
        text: t('withdrawRecords.statusFailed'),
  ...
  return (
    <div className='flex w-full flex-col gap-3'>
      ...
      <div className='flex items-center gap-[10px]'>
        <h2 className='text-base font-medium text-white/80'>{t('withdrawRecords.title')}</h2>
      </div>

      {/* Records Table */}
      <div className='bg-card flex flex-col rounded-xl'>
        {isLoading && records.length === 0 ? (
          <ContentLoading text={t('withdrawRecords.loading')} className='py-10' />
        ) : records.length === 0 ? (
          <div className='flex items-center justify-center py-10'>
            <span className='text-sm text-white/60'>暂无提现记录</span>
          </div>
        ) : (
              ...
              <div className='flex items-stretch justify-stretch'>
                ...
                <div className='flex flex-1 items-center gap-1 px-4 py-2'>
                  <span className='text-xs font-medium text-white/80 flex-shrink-0'>
                    {t('withdrawRecords.time')}：{formatTime(record.createTime)}
                  </span>
                </div>
                ...
                <div className='flex flex-1 items-center gap-1 px-4 py-2'>
                  <span className='text-xs font-medium text-white/80 flex-shrink-0'>
                    {t('withdrawRecords.network')}：{record.network || '-'}
                  </span>
                </div>
                ...
                <div className='flex flex-1 items-center gap-1 px-4 py-2'>
                  <span className='text-xs font-medium text-white/80  flex-shrink-0'>
                    {t('withdrawRecords.address')}：{truncateAddress(record.toAddress || '')}
                  </span>
                  {record.toAddress && (
                    <button
