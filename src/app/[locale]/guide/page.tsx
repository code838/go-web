...
export default function GuidePage() {
	...
	// 查找新手指南 (type: 3)
	const guideProtocol = protocols?.find(p => p.type === 3)

	// 清理HTML内容，移除可能影响页面样式的标签
	const cleanedContent = useMemo(() => {
		if (!guideProtocol?.content) return ''

		let content = guideProtocol.content
